...
# ----------------------------------------
# Sensitivity analysis: inclusion criteria 
# of baseline SER >= -4.00 D
# ----------------------------------------

rm(list=ls())

save_path         <- "C:/past_grogression_"
save_version      <- "3_sensitivity_incl_minus_four"
...
data3                  <- data2[which(data2$AgeDiffPRE >=0.5 & data2$AgeDiffPOST >=0.5 & data2$SER1 < -0.50 & data2$SER1 >= -4.00),
...
# Combined graph of original and sensitivity analysis (Figure S2)

save_version              <- "3"
in_file                   <- paste0(save_path,"denmark_results_v",save_version,".csv")
res_orig                  <- read.csv(file=in_file, header=TRUE)
save_version              <- "3_sensitivity_incl_minus_four"
in_file                   <- paste0(save_path,"denmark_results_v",save_version,".csv")
res_sens                  <- read.csv(file=in_file, header=TRUE)

res_orig1                 <- res_orig[which(res_orig$BaselineAge > 11 & res_orig$Threshold == "treatment_perc"),]
res_orig1$signif          <- ifelse(res_orig1$Pval < 0.05, 1, 0)
res_orig_sum              <- as.data.frame(res_orig1 %>% group_by(Selection_trait,RCT_trait,Responder,BaselineAge) %>% 
                                           get_summary_stats(signif, type="mean_ci"))
mylab                     <- paste0((effect_perc*100),"% slowing")
res_orig_sum$RCT_trait         <- ordered(res_orig_sum$RCT_trait, levels=c("RCT_for_AXL","RCT_for_SER"))
res_orig_sum$RCT_trait         <- revalue(res_orig_sum$RCT_trait, c("RCT_for_AXL"="RCT outcome trait: AL", "RCT_for_SER"="RCT outcome trait: SER"))
res_orig_sum$Selection_trait   <- ordered(res_orig_sum$Selection_trait, levels=c("unselected","select_on_axl","select_on_ser"))
res_orig_sum$Selection_trait   <- revalue(res_orig_sum$Selection_trait, c("unselected"="Unselected", "select_on_axl"="Fast AL prog.",
                                                                                           "select_on_ser"="Fast SER prog."))
res_orig_sum$Responder         <- ordered(res_orig_sum$Responder, levels=c("all","half","mixed"))
res_orig_sum$Responder         <- revalue(res_orig_sum$Responder, c("all"="All respond equally", "half"="Half respond; Half do not",
                                                                                           "mixed"="All respond, but variably")) 

res_sens1                 <- res_sens[which(res_sens$Threshold == "treatment_perc"),]
res_sens1$signif          <- ifelse(res_sens1$Pval < 0.05, 1, 0)
res_sens_sum              <- as.data.frame(res_sens1 %>% group_by(Selection_trait,RCT_trait,Responder,BaselineAge) %>% 
                                           get_summary_stats(signif, type="mean_ci"))
mylab                     <- paste0((effect_perc*100),"% slowing")
res_sens_sum$RCT_trait         <- ordered(res_sens_sum$RCT_trait, levels=c("RCT_for_AXL","RCT_for_SER"))
res_sens_sum$RCT_trait         <- revalue(res_sens_sum$RCT_trait, c("RCT_for_AXL"="RCT outcome trait: AL", "RCT_for_SER"="RCT outcome trait: SER"))
res_sens_sum$Selection_trait   <- ordered(res_sens_sum$Selection_trait, levels=c("unselected","select_on_axl","select_on_ser"))
res_sens_sum$Selection_trait   <- revalue(res_sens_sum$Selection_trait, c("unselected"="Unselected", "select_on_axl"="Fast AL prog.",
                                                                                           "select_on_ser"="Fast SER prog."))
res_sens_sum$Responder         <- ordered(res_sens_sum$Responder, levels=c("all","half","mixed"))
res_sens_sum$Responder         <- revalue(res_sens_sum$Responder, c("all"="All respond equally", "half"="Half respond; Half do not",
                                                                                           "mixed"="All respond, but variably")) 

plot5 <- ggplot(res_orig_sum, aes(x=BaselineAge,y=mean,colour=Selection_trait))+
  theme(panel.grid.major = element_line(linetype="dotted", colour = "grey", linewidth = 0.5)) +
  coord_cartesian(ylim=c(0,1))+
  labs(x="Age at baseline (years)",y="Statistical power")+
  ggtitle("Original analysis (as Figure 1)")+
  scale_y_continuous(breaks=c(0,0.2,0.4,0.6,0.8,1),labels = scales::percent)+
  scale_colour_manual(values=c("dark red","blue","orange"),name="Selection scheme")+
  geom_point(size=2, position=position_dodge(width=0.2))+
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), position=position_dodge(width=0.2), linewidth=0.6, width=0)+
  ...
  facet_grid( ~ RCT_trait ~ Responder)
plot5
plot6 <- ggplot(res_sens_sum, aes(x=BaselineAge,y=mean,colour=Selection_trait))+
  theme(panel.grid.major = element_line(linetype="dotted", colour = "grey", linewidth = 0.5)) +
  coord_cartesian(ylim=c(0,1))+
  labs(x="Age at baseline (years)",y="Statistical power")+
  ggtitle("Sensitivity analysis (baseline SER >= -4.00 D)")+
  scale_y_continuous(breaks=c(0,0.2,0.4,0.6,0.8,1),labels = scales::percent)+
  scale_colour_manual(values=c("dark red","blue","orange"),name="Selection scheme")+
  geom_point(size=2, position=position_dodge(width=0.2))+
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), position=position_dodge(width=0.2), linewidth=0.6, width=0)+
...
save_version   <- "3_sensitivity_incl_minus_four"
out_file=paste0(save_path,"figS2_v",save_version,".tiff")
all_plots      <- plot_grid(plot6, plot5, ncol=1, labels=c('A','B'))
ggsave(out_file, units="cm", width=18, height=22, dpi=300, compression = 'lzw')
dev.off()
